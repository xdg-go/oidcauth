# oidcauth: Sliding Session Renewal with Absolute Cap
...
session payload, slide the idle window on the verify path, cap the
total lifetime, and expose issue time to applications. The skeleton
half (per-user revocation epoch) lives in the template repo and depends
on the release this plan produces.

Target end state for apps: a 90d idle window with sliding renewal, a 1y
absolute cap, and enough exposed session metadata for an app to reject
...
- **Automated tests for session logic**: payload marshal/unmarshal,
  ...
  absolute-cap refusal. This is security-relevant code -- every branch
  gets a test.
...
Phase 2 (IssuedAt in payload, fail closed)
       │
       ├─► Phase 3 (Sliding renewal + absolute cap)
       │            │
       │            ▼
...
## Phase 3: Sliding Renewal and Absolute Cap

Two clocks: an idle window that slides, and an absolute lifetime that
does not.

### 3.1 Configuration surface
- [ ] Add idle-window and absolute-cap options (names and defaults
      decided here; recommended library defaults: 90d idle, 1y cap)
- [ ] Validate at construction: cap must exceed the idle window; a
      non-positive value is a config error, not a silent default
- [ ] Keep the single-TTL behavior reachable (cap == idle window)
- [ ] **Test**: construction rejects cap < idle window
- [ ] Name the middleware `Authenticate` (renews) and
      `AuthenticateNoRenew` (verifies only); `RequireAuth` composes on
      `Authenticate`; delete `Auth.User(r)`
- [ ] `Authenticate` always stores a context sentinel, even for
      anonymous requests, so "middleware never ran" is distinguishable
      from "not logged in"
- [ ] `RequireAuth` verifies inline when the sentinel is absent, so both
      `Authenticate(mux)` + `RequireAuth` and bare `RequireAuth` are
      correct with exactly one verification
- [ ] **Test**: bare `RequireAuth` (no outer `Authenticate`) still
      authenticates, and does not verify twice when wrapped

### 3.2 Renewal in the verify path
- [ ] Re-issue the cookie when now is past `Expiry - idleWindow/2`
      (half-life of the idle window), preserving the original
      `IssuedAt`
- [ ] Refuse renewal -- and reject the request -- once now is past
      `IssuedAt + absoluteCap`, even if the idle window has not lapsed
- [ ] Renewal writes `Set-Cookie` at middleware entry, before calling
      the next handler, so it cannot lose a race with the handler's own
      headers (see 3.4)
- [ ] **Test**: fresh cookie, no renewal, no `Set-Cookie`
- [ ] **Test**: just past half-life, renewed, new `Expiry`, same
      `IssuedAt`
- [ ] **Test**: past idle window, rejected
- [ ] **Test**: within idle window but past absolute cap, rejected and
      not renewed
- [ ] **Test**: renewal on a request whose handler writes a response
      body -- cookie still lands
- [ ] Clamp a renewed `Expires` to `IssuedAt + absoluteCap` so the cookie
      never advertises a lifetime the server will refuse to honor
- [ ] Suppress the duplicate `Set-Cookie` when renewal races a mint or a
      clear: `setSessionCookie` and `clearSessionCookie` must drop any
      pending same-name `Set-Cookie` from `w.Header()` before writing
- [ ] **Test**: logout past the renewal half-life emits exactly one
      `Set-Cookie` for the session cookie, and it is the clearing one
- [ ] **Test**: the callback path past half-life likewise emits one

### 3.3 Concurrency and races
- [ ] **Test**: `-race` over concurrent requests carrying the same
      cookie; renewal must not corrupt shared state
- [ ] Confirm renewal is not attempted on requests where the session is
...
- [ ] Set `Cache-Control: private, no-store` on every response where the
      library writes a session cookie: renewal, callback, and logout
- [ ] Set `Cache-Control: private` when a valid session is found but no
      cookie is written (both `Authenticate` and `AuthenticateNoRenew`)
- [ ] Leave `Cache-Control` untouched on anonymous requests, so public
      pages stay shared-cacheable
- [ ] Add `Vary: Cookie` unconditionally at middleware entry (see the
      decision log; one `Header().Add`, no branch)
- [ ] Document that a handler which overwrites `Cache-Control` on a
      renewed response defeats this, and that the library does not
      prevent it
- [ ] **Test**: a renewed response carries `private, no-store`
- [ ] **Test**: a valid session with no renewal carries `private` and no
      `no-store`
- [ ] **Test**: an anonymous request leaves a handler-set
      `Cache-Control: public` intact
- [ ] **Test**: `Vary: Cookie` present on all of the above
...
- [ ] Document that an idle-window change applies only to cookies minted
      or renewed afterward; the cap, keyed on stored `IssuedAt`, does
      apply retroactively
...
- [ ] Document the rotation procedure: move the current secret into
      ...
      the old key one full idle window later -- by then every live
      session has renewed at least once and been re-signed, and a session
      idle for almost the whole window is the worst case that still needs
      the old key
...
- [ ] Note explicitly that renewal keeps a stolen cookie alive only
      until the absolute cap, and that revocation is the app's job
- [ ] State that renewal is not re-authentication: claims freeze at
      login, so a user disabled at the IdP can stay valid until the cap
- [ ] `AuthenticateNoRenew` doc comment warns that at least one renewing
      mount must sit in the user's regular path, or sessions expire at
      the idle window regardless of activity
