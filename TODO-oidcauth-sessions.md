...
- [x] Add `WithRevokedBefore(fn func(ctx context.Context, u User)
      (time.Time, error)) Option`; nil fn is a config error. Remove
      `WithSessionValidator` outright -- nothing is tagged yet
- [x] Rename the field and helpers (`sessionValidator`,
...
- [x] In `verifyOnce`: call the hook after the built-in checks; on
...
- [x] **Test**: hook not called when the cookie is absent, malformed,
      expired, or past max lifetime
- [x] Port every 4.2/4.3 test to the new signature; delete tests that
      only exercised `(false, nil)`
- [x] Rewrite the `WithRevokedBefore` doc: three outcomes on the value
...
- [x] Rewrite the package doc's "Revoking sessions": store `time.Now()`
...
- [x] README: update the revocation example to the new hook
- [x] Update the skeleton's revocation-epoch code to return the stored
      time instead of comparing it (N/A: the skeleton has no
      revocation code yet; it will be written against `WithRevokedBefore`)
